...
export default class Chat extends Component {
    constructor(props){
        ...
        this.state = {
            ...
            response: [],
            inResponse:[],
            conversation: 'Bastiat: Hello there, do you have a moment to chat?',
        }
    }
    ...
    componentDidMount(){
        axios.get('/chat').then(response=>{
            ...
            var response2 = response.data.filter(a=>{return ((a.bot==='all'||a.bot==='Bastiat')&&(a.hasOwnProperty('inResponse')))})
            response = response.data.filter(a=>{return (a.bot==='all'||a.bot==='Bastiat')&&(a.inResponse===null||a.inResponse===undefined)})
    ...
    render(){
        ...
        const handleSubmit = (e) =>{
            ...
            if(this.state.chat[0]==='/' && this.state.chat[1]!=='/'){
                ...
                axios.post('/chat', {text:this.state.chat.slice(1), bot:'Bastiat'})
                ...
                axios.post('/chat', {text:this.state.chat.slice(this.state.chat.indexOf('//',2)+2), bot:'Bastiat', inResponse:this.state.chat.slice(2,this.state.chat.indexOf('//',2))})
            }else {
                ...
                if(inres.length>0){
                    ...
                    this.setState(prevState=>{
                        return {
                            conversation: prevState.conversation+'\n\nYou: '+prevState.chat+'\n\nBastiat: '+inres[Math.floor(Math.random()*inres.length)].text+'\n',
                            chat: '',
                        }
                    })
                } else{
                    ...
                    this.setState(prevState=>{
                        return {
                            conversation: prevState.conversation+'\n\nYou: '+prevState.chat+'\n\nBastiat: '+prevState.response[Math.floor(Math.random()*prevState.response.length)]+'\n',
                            chat: '',
                        }
                    })
                }
